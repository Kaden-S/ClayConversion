...
public class Config {
	...
	static {
		...
		cfgBuilder.comment("General settings").push(generalCategory);
		{
			...
			{
				clayRecipeEnabled = cfgBuilder.comment("Enable Clay Recipe [Default: true]").define("clay_recipe",
						true);
				snowRecipeEnabled = cfgBuilder.comment("\nEnable Snow Recipe [Default: true]").define("snowball_recipe",
						true);
				quartzRecipeEnabled = cfgBuilder.comment("\nEnable Quartz Recipe [Default: true]")
						.define("quartz_recipe", true);
